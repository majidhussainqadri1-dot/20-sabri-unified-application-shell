...
- Before any File01 write, deploy the exact 1.4.16 candidate and prove deployed File20 runtime/package parity.
- Run File01 reconciliation dry-run only. `home` and `news` must remain accepted by `file-21`.
- The twelve File20 keys—`founder`, `learn`, `encyclopedia`, `doctors`, `clinic`, `videos`, `reels`, `pdf`, `radar`, `ai`, `network`, `marketplace`—must each return an accepted `file-20` owner plan with command version `1.0.1`.
...
- Verify the trusted persistence correction in the exact deployed runtime: adapter execute/compensation/rollback must use the bounded writer that temporarily removes only the registered `Settings::sanitize` callback, calls `Settings::enforce_owned_invariants`, preserves every other filter and restores the sanitizer in `finally`.
...
- Verify all twelve routes persist their expected Page IDs and still resolve from File20-owned configured Page IDs after `spf_page_map` removal. Verify Home/News remain File21-owned and no companion page/content/table is copied, rewritten or re-owned by File20.
- Rehearse rollback where permitted: exact prior File20 navigation rows must restore under the same trusted persistence path; the Settings sanitizer must be restored; plan-hash/receipt/state drift must fail closed; replay must be idempotent.
...
- After deploying the exact approved 1.4.16 artifact, confirm live runtime reports `1.4.16`.
...
**Pass condition:** zero known unresolved release-blocking defects, including zero File01 reconciliation blockers before apply and successful persisted reconciliation under the active Settings API sanitizer, or an explicitly permitted and documented Founder risk acceptance. Staging acceptance still does not equal Live-Deployed or Operational status.
