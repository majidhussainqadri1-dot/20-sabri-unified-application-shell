...
final class SystemCheck {
	...
	public static function report() {
		...
		$rows         = array(
			...
			self::row( 'activation-snapshot', __( 'Activation snapshot', 'sabri-unified-application-shell' ), get_option( Defaults::SNAPSHOT_OPTION_NAME, false ) ? __( 'Captured', 'sabri-unified-application-shell' ) : __( 'Not captured yet', 'sabri-unified-application-shell' ), get_option( Defaults::SNAPSHOT_OPTION_NAME, false ) ? 'pass' : 'warn', get_option( Defaults::SNAPSHOT_OPTION_NAME, false ) ? 'info' : 'high', 'File 20 activation snapshot option.', $now, 'recovery', __( 'Create and integrity-check a File 20-owned snapshot before repair/upgrade.', 'sabri-unified-application-shell' ) ),
	...
	private static function duplicate_shell_plugins() {
		...
		$matches = array_filter( $active, static function ( $plugin ) { return false !== strpos( $plugin, 'sabri-global-ui' ); } );
		$unified = array_filter( $active, static function ( $plugin ) { return false !== strpos( $plugin, 'sabri-unified-application-shell' ); } );
		if ( count( $unified ) > 1 ) {
