...
foreach ( $features as $feature ) {
    $assert( false !== strpos( $php, "'{$feature}'" ), 'feature ' . $feature );
}

$assert( false !== strpos( $main, '* Version: 1.4.7' ) && false !== strpos( $main, "define( 'SABRI_SHELL_VERSION', '1.4.7' );" ), 'release 1.4.7' );
foreach ( array(
    ...
    'FutureShellV5SeventhHardening::register();'
) as $registration ) {
...
foreach ( array( 'CF-01','CF-02','CF-03','CF-04','CF-05','CF-06' ) as $cf ) {
    $assert( false !== strpos( $seventh, "\$registry['{$cf}']" ), 'conditional registry ' . $cf );
}
...
$all_shell = $php . $hard . $client . $control . $second . $third . $fourth . $fifth . $sixth . $seventh;
$assert( false === strpos( $all_shell, 'CREATE TABLE' ) && false === strpos( $all_shell, 'dbDelta(' ) && false === strpos( $all_shell, 'INSERT INTO' ), 'no foreign database backend anywhere in Future Shell layers' );

if ( $fail ) {
    fwrite( STDERR, "Future Shell v5 comprehensive FAIL: " . implode( '; ', $fail ) . "\n" );
    exit( 1 );
}
echo "Future Shell v5: 18/18 enhancements + seven corrective passes + CF-01..CF-06 conditional harmonization PASS\n";
