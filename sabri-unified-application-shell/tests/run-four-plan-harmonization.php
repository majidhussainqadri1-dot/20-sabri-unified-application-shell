...
$checks = array();
$assert = static function ($condition, $label) use (&$checks) { if (!$condition) { fwrite(STDERR, "FAIL: {$label}\n"); exit(1); } $checks[] = $label; };
...
$tenth = file_get_contents($root . '/includes/class-future-shell-v5-tenth-hardening.php');
...
$assert(strpos($main, 'FutureShellV5NinthHardening::register') !== false && strpos($ninth, "CONTRACT_VERSION = '1.0.9'") !== false, 'ninth corrective harmonization preserved');
$assert(strpos($main, 'FutureShellV5TenthHardening::register') !== false && strpos($tenth, "CONTRACT_VERSION = '1.0.10'") !== false, 'tenth corrective harmonization registered');
...
$assert(strpos($tenth, 'file26_search_surface_only') !== false && strpos($tenth, "'file20_fallback' => false") !== false, 'File20 provides no WordPress/local search fallback');
$assert(strpos($central, "'26' => array( 'Search Discovery and Ranking'") !== false, 'canonical registry includes File26');
foreach (array('CF-01','CF-02','CF-03','CF-04','CF-05','CF-06') as $cf) { $assert(strpos($seventh, "\$registry['{$cf}']") !== false, 'conditional registry includes ' . $cf); }
