...
$assert( false !== strpos( $main, '* Version: 1.4.7' ) && false !== strpos( $main, "SABRI_SHELL_VERSION', '1.4.7" ), 'release identity 1.4.7' );
...
foreach ( array( 'CF-01', 'CF-02', 'CF-03', 'CF-04', 'CF-05', 'CF-06' ) as $id ) {
	$assert( false !== strpos( $seventh, "\$registry['{$id}']" ), 'conditional registry includes ' . $id );
}
...
foreach ( array(
	'/clinic/records', '/clinic/patients', '/my-health-record', '/clinic/encounters', '/clinic/prescriptions', '/clinic/follow-ups',
	'/support/cases', '/support/appeals', '/admin/support',
	'/checkout', '/billing', '/admin/finance',
	'/api/media/v1', '/admin/media',
	'/insights', '/admin/analytics',
	'/settings/language', '/admin/localization'
) as $path ) {
	$assert( false !== strpos( $seventh, "'{$path}'" ), 'private conditional path ' . $path );
}
...
if ( $fail ) {
	fwrite( STDERR, "Future Shell v5 seventh hardening FAIL: " . implode( '; ', $fail ) . "\n" );
	exit( 1 );
}

echo "Future Shell v5 seventh ten-round hardening: CF-01..CF-06 conditional contracts, privacy boundaries, financial law, media cache authority and no foreign backend PASS\n";
