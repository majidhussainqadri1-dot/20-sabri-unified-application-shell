...
$assert( false !== strpos( $main, '* Version: 1.4.11' ) && false !== strpos( $main, "SABRI_SHELL_VERSION', '1.4.11" ), 'current release identity 1.4.11 preserves fourth hardening' );
...
foreach ( array( 'normal','elevated-monitoring','restricted-high-risk-actions','upload-lockdown','identity-lockdown','publishing-read-only','platform-read-only','incident-containment' ) as $state ) { $assert( false !== strpos( $fourth, "'{$state}'" ), 'File 24 state ' . $state ); }
...
echo "Future Shell v5 fourth hardening preserved under 1.4.11: historical companion contracts, public WebAuthn route and File24 states PASS\n";
